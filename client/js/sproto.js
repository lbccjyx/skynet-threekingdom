    ...
    dispatch(buffer){
        ...
        if (this.headerTmp.type != null){
            ...
            if (this.headerTmp.session != null){
                const sessionKey = BigInt(this.headerTmp.session);
                let proto = this.sessions.get(sessionKey);
                if (proto != null){
...
class Sproto {
    constructor(schema) {
        ...
        if (schema) {
            ...
            if (typeof schema === 'string') {
                const SprotoParser = window.SprotoParser;
    ...
    _decode(args){
        ...
        switch(args.type){
            case SPROTO_TINTEGER:{
                let v = BigInt.asIntN(64, args.value);
                if (args.extra){
                    value = Number(v) / args.extra;
                } else {
